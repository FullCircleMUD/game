"""
Tests for the defence command (STRATEGY skill — group defensive stance).

evennia test --settings settings tests.command_tests.test_cmd_defence
"""
...
from commands.class_skill_cmdsets.class_skill_cmds.cmd_defence import (
    CmdDefence, DEFENCE_SCALING,
)
...
class _DefenceTestBase(EvenniaCommandTest):
    """Shared setUp/tearDown for defence tests."""
...
class TestDefenceGates(_DefenceTestBase):
    ...
    def test_follower_cannot_set_stance(self, mock_ticker):
        ...
        try:
            ...
            result = self.call(CmdDefence(), "", caller=self.char1)
...
class TestDefenceMechanics(_DefenceTestBase):
    """Test defence stance mechanics."""
    ...
    @patch("combat.combat_handler.TICKER_HANDLER")
    def test_defence_applies_stat_bonuses(self, mock_ticker):
        """Defence applies AC bonus and hit penalty."""
        self._set_mastery(self.char1, MasteryLevel.BASIC)
        ...
        scaling = DEFENCE_SCALING[MasteryLevel.BASIC]
    ...
    @patch("combat.combat_handler.TICKER_HANDLER")
    def test_defence_toggle_removes(self, mock_ticker):
        """Calling defence again removes the stance."""
        self._set_mastery(self.char1, MasteryLevel.BASIC)
        enter_combat(self.char1, self.mob)
    ...
    @patch("combat.combat_handler.TICKER_HANDLER")
    def test_defence_replaces_offence(self, mock_ticker):
        """Activating defence removes active offensive stance."""
        self._set_mastery(self.char1, MasteryLevel.BASIC)
        enter_combat(self.char1, self.mob)
    ...
    @patch("combat.combat_handler.TICKER_HANDLER")
    def test_defence_applies_to_group(self, mock_ticker):
        """Defence applies to all group members in combat."""
        self._set_mastery(self.char1, MasteryLevel.BASIC)
        enter_combat(self.char1, self.mob)
        ...
        self.assertTrue(self.char1.has_effect("defensive_stance"))
        self.assertTrue(self.char2.has_effect("defensive_stance"))

        self.char2.following = None

    @patch("combat.combat_handler.TICKER_HANDLER")
    def test_grandmaster_no_hit_penalty(self, mock_ticker):
        """Grandmaster defence has no hit penalty."""
        ...
        scaling = DEFENCE_SCALING[MasteryLevel.GRANDMASTER]
        self.assertEqual(scaling["hit"], 0)
