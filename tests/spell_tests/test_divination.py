"""
Tests for the divination spell school.

Tests:
    - Registry: all divination spells registered with correct attributes
    - Identify: actor template, mastery gate, PvP check, NFT item identification
    - Augur: actor identification, mastery gate, PvP check
...
evennia test --settings settings tests.spell_tests.test_divination
"""
...
class TestDivinationRegistry(EvenniaTest):
    ...
    def test_identify_registered(self):
        ...
        self.assertEqual(spell.target_type, "items_inventory_then_room_all")
...
class TestIdentify(EvenniaTest):
    ...
    Identify is now item-only (target_type="items_inventory_then_room_all"). Actor
