...
Every test in the original file exercises ZoneSpawnScript behaviour
(now retired in favour of the evennia-mob-spawner library) plus one
auxiliary `TestCombatMobDeath` class that tests `mob.die()` deletes
the mob. The death-notify subtest in that class also relied on the
ZoneSpawnScript callback path which is now a no-op.

The original 555-line file is preserved as a comment block below so
it can be restored from this file alone if a regression surfaces.
When this whole file is removed, the surviving generic
`test_mob_deleted_on_death` should be rehomed into a new
`tests/typeclass_tests/test_mob_death.py` rather than carried back.
"""
