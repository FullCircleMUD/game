...
The workhorse utility spell for divination. Reveals stats/properties
of actors and NFT items via dynamic templates.

Implemented branches:
    - Actors (mobs/PCs): level-gated dynamic stat template
    - NFT items (weapons, armor, holdables, consumables, containers):
      dynamic property template, per-item mastery gate via
      identify_mastery_gate attribute (default BASIC)

All other objects get a sassy one-liner (mana consumed, no refund).

Actor mastery gate is level-based — higher-level targets require
higher divination mastery to identify:
...
def _required_tier(target_level):
    """Return the minimum divination tier needed to identify an actor."""
...
class Identify(Spell):
    ...
    target_type = "any"
    cooldown = 0
    description = "Reveals hidden properties of items and creatures."
    mechanics = (
        "Utility spell — reveals information, no damage.\n"
        "Syntax: cast identify <item or creature>\n"
    ...
    def _execute(self, caster, target):
        ...
        # Classify target — inline imports to avoid circular deps
        from typeclasses.actors.base_actor import BaseActor
        from typeclasses.items.base_nft_item import BaseNFTItem

        if isinstance(target, BaseActor):
            return self._identify_actor(caster, target, tier)

        if isinstance(target, BaseNFTItem):
            return self._identify_item(caster, target, tier)
    ...
    def _identify_actor(self, caster, target, tier):
        ...
        required = _required_tier(target_level)
        if tier < required:
        ...
        # Build the identify output
        lines = []
        lines.append(f"|w--- Identify: {target.key} ---|n")

        if is_pc:
            self._build_pc_header(target, lines)
        else:
            self._build_mob_header(target, lines)

        self._build_vitals(target, lines)
        self._build_ability_scores(target, lines)

        if is_pc:
            self._build_pc_combat(target, lines)
        else:
            self._build_mob_combat(target, lines)

        self._build_resistances(target, lines)
        self._build_conditions(target, lines)
        self._build_effects(target, lines)

        if is_pc:
            self._build_memorised_spells(target, lines)
    ...
    def _build_pc_header(self, target, lines):
    ...
    def _build_mob_header(self, target, lines):
    ...
    def _build_vitals(self, target, lines):
        """HP, Mana, Move, AC."""
    ...
    def _build_ability_scores(self, target, lines):
    ...
    def _build_mob_combat(self, target, lines):
    ...
    def _build_pc_combat(self, target, lines):
    ...
    def _build_resistances(self, target, lines):
    ...
    def _build_conditions(self, target, lines):
    ...
    def _build_effects(self, target, lines):
    ...
    def _build_memorised_spells(self, target, lines):
    ...
    def _identify_item(self, caster, target, tier):
        ...
        # Item mastery gate — some items require higher divination to identify
        ...
        if isinstance(target, WeaponNFTItem):
            self._build_weapon_info(target, lines)
        elif isinstance(target, HoldableNFTItem):
            self._build_holdable_info(target, lines)
        elif isinstance(target, WearableNFTItem):
            self._build_wearable_info(target, lines)
        elif isinstance(target, PotionNFTItem):
            self._build_potion_info(target, lines)
        elif isinstance(target, SpellScrollNFTItem):
            self._build_scroll_info(target, lines)
        elif isinstance(target, CraftingRecipeNFTItem):
            self._build_recipe_info(target, lines)
        elif isinstance(target, ContainerNFTItem):
            self._build_container_info(target, lines)
        else:
            lines.append("|cType:|n Item")

        self._build_item_weight(target, lines)
        self._build_item_restrictions(target, lines)
    ...
    def _build_weapon_info(self, target, lines):
        ...
        self._build_durability(target, lines)
        self._build_wear_effects_line(target, lines)

    def _build_wearable_info(self, target, lines):
        """Armor/clothing/jewelry: slot, durability, effects."""
        slot_str = self._format_slot(getattr(target, "wearslot", None))
        lines.append(f"|cType:|n Armor ({slot_str})")
        self._build_durability(target, lines)
        self._build_wear_effects_line(target, lines)

    def _build_holdable_info(self, target, lines):
        """Shield/orb/torch: durability, effects."""
        lines.append("|cType:|n Holdable")
        self._build_durability(target, lines)
        self._build_wear_effects_line(target, lines)

    def _build_potion_info(self, target, lines):
    ...
    def _build_scroll_info(self, target, lines):
    ...
    def _build_recipe_info(self, target, lines):
    ...
    def _build_container_info(self, target, lines):
    ...
    def _build_durability(self, target, lines):
    ...
    def _build_wear_effects_line(self, target, lines):
        """Format wear_effects into a single Effects line."""
        effects = getattr(target, "wear_effects", []) or []
        parts = self._format_wear_effects(effects)
        if parts:
            lines.append(f"|cEffects:|n {', '.join(parts)}")
    ...
    def _format_slot(wearslot):
        ...
        if isinstance(wearslot, list):
            # Multi-slot items — use the first slot's name
            if wearslot:
        ...
        # Convert "LEFT_RING_FINGER" → "Left Ring Finger"
        return val.replace("_", " ").title()

    def _build_item_weight(self, target, lines):
        """Weight line."""
        weight = getattr(target, "weight", 0) or 0
        lines.append(f"|cWeight:|n {weight} kg")

    def _build_item_restrictions(self, target, lines):
