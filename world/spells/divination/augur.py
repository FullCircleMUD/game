"""
Augur — divination spell, available from BASIC mastery.

Actor-only identification spell. Reveals stats, abilities, resistances,
conditions, and effects of creatures and players via dynamic templates
...
class Augur(Spell):
    ...
    cooldown = 0
    description = "Reveals the nature and capabilities of a creature."
    mechanics = (
        "Utility spell — reveals information, no damage.\n"
        "Syntax: cast augur <creature>\n"
        ...
        "For items, use the Identify spell instead.\n"
        "No cooldown."
    )
    ...
    def _execute(self, caster, target):
        ...
        # inspect_actor handles PvP gate and mana refund internally,
        # but it needs the mana_cost dict for the refund amount.
        result = inspect_actor(caster, target, tier)

        # If PvP gate returned a hard failure string, the spell framework
        # expects (False, str) — refund mana here since inspect_actor
        # doesn't know the spell's cost table.
