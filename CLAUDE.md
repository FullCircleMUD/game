...
- **Combat expansion** — multi-attack (attacks_per_round > 1), combat skill commands (bash, assassinate, etc.), spell casting in combat, combat prompt/status display, LLM-driven mob AI. **Height combat (Phase 1 done):** melee blocked across height differentials, missile weapons work cross-height, -4 to-hit for ranged at same height (melee range penalty). Mid-combat height changes: players get "can't reach" + skip tick, mobs retarget to reachable enemy or flee if none. See `combat/height_utils.py`. Future phases: mob AI aggro filtering, mob ranged weapons, flee height mechanics, spell height interactions.
