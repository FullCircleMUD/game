...
- **Combat expansion** — multi-attack (attacks_per_round > 1), combat skill commands (bash, assassinate, etc.), spell casting in combat, combat prompt/status display, LLM-driven mob AI. **Height combat (Phases 1-3 done):** melee blocked across height differentials, missile weapons work cross-height, -4 to-hit for ranged at same height (melee range penalty). Mid-combat height changes: players get "can't reach" + skip tick, mobs retarget to reachable enemy or flee if none. Mob aggro is height-aware: flying mobs (`Condition.FLY`) adjust `room_vertical_position` to match targets before aggro, swimming mobs use `Condition.WATER_BREATHING` for underwater targets (see `AggressiveMob._try_match_height()`). Flee auto-succeeds when no enemy can melee the fleeing character (all enemies at different height or ranged-only). See `combat/height_utils.py`. Future phases: mob ranged weapons, spell height interactions.
