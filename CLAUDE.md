...
│   │   │   ├── npcs/                 ← NPC subtypes — hierarchy:
│   │   │   │                            ShopkeeperNPC (abstract) → ResourceShopkeeperNPC / NFTShopkeeperNPC
│   │   │   │                            LLMResourceShopkeeperNPC / LLMNFTShopkeeperNPC add LLM layer
│   │   │   │                            TrainerNPC / LLMTrainerNPC (LLM-powered variant)
│   │   │   │                            GuildmasterNPC / LLMGuildmasterNPC
│   │   │   │                            TutorialGuideNPC, LLMRoleplayNPC, BartenderNPC, BakerNPC (quest-giving shop)
│   │   │   │                            Mara/Oakwright/Torben/Hendricks/Elena — explicit composition of
│   │   │   │                            (LLMQuestContextMixin, QuestGiverMixin, LLMTrainerNPC [+ shop])
...
**Syscommand overrides:** `CmdNoMatch` (`cmd_override_nomatch.py`) overrides Evennia's default `CMD_NOMATCH` system command to emit a terse "Huh?!?" for unknown input, replacing the default "Command 'x' is not available. Maybe you meant..." with fuzzy suggestions.
...
- **Shopkeeper commands** (`cmdset_resource_shop.py`, `cmdset_nft_shop.py`) — call abstract methods on the NPC (which in turn call AMMService / NFTAMMService) via worker threads
...
**Commands using this pattern:** `cmd_wallet.py`, `cmd_export.py`, `cmd_import.py`, `cmdset_resource_shop.py`, `cmdset_nft_shop.py`, `cmd_override_unconnected_connect.py`, `cmd_sync_nfts.py`, `cmd_sync_reserves.py`, `cmd_reconcile.py`.
...
- Text prompt system: DikuMUD-style inline prompt rendered in scrollback (not a side pane). `FCMCharacter.msg()` override schedules a bare prompt line after any text burst, debounced via `reactor.callLater(0, ...)` so N messages in one reactor tick collapse to exactly one trailing prompt. `FCMCommandMixin.at_pre_cmd()` echoes `{prompt}> {raw_string}` before the command runs so the typed input appears in scrollback. Both paths are gated on `reactor.running` so the test harness stays clean. Token-based format string resolved by `_PROMPT_TOKENS` dict on `FCMCharacter`: vitals (`%h/%H` HP, `%m/%M` Mana, `%v/%V` Move), autowarn colour-forced vitals (`%i` HP, `%n` Mana, `%w` Move, `%s` self status using diagnose tier words — all G/Y/R based on ratio), character/world (`%g` Gold, `%x` XP, `%l` Level, `%a` AC, `%A` Alignment, `%C` Position, `%T` Time of day descriptor, `%r` newline), and battle-only (`%f` target name, `%c` target condition — empty string outside combat). `%s`/`%c` share `utils/health_desc.py` with the `diagnose` command. Default: `"%hH %mM %vV > "`. `prompt` command to customise, `toggle prompt` to disable. `at_post_cmd` no longer sends `msg(prompt=...)` — the legacy `#prompt` div was removed from the webclient template. OOB vitals panel still drives the split-screen vitals display.
...
- Wand system: two-stage crafting pipeline. **Stage 1 (carpenter):** five tiers of inert blank wands carved from timber/ironwood in a woodshop — Training (BASIC) / Apprentice's (SKILLED) / Wizard's (EXPERT) / Master's (MASTER) / Archmage's (GRANDMASTER). Recipe scrolls required to learn each tier. Blanks live as plain `BaseNFTItem` components in `world/prototypes/components/`, conceptually identical to shaft/haft. **Stage 2 (mage):** in a wizard's workshop, a mage with the spell currently in their spellbook AND current school mastery >= the spell's tier consumes a tier-matched blank + 2 Arcane Dust to produce a `Wand of <spell>` (`WandNFTItem`). Recipes are **dynamic** — generated at `get_known_recipes()` time from (blanks-in-inventory × spells-in-spellbook) by `RecipeBookMixin._get_wand_enchant_recipes()`. Charges are fixed by spell tier (BASIC=10, SKILLED=8, EXPERT=6, MASTER=4, GM=2). Mana for every charge is **pre-paid** at enchant time; zaps cost zero mana. **`zap <target>`** command: requires the wand in the HOLD slot (no ambiguity), fires the bound spell via `spell.cast()` with two NDB overrides on the caster — `_wand_caster_tier_override` forces the spell to cast at its base min_mastery regardless of zapper, and `_wand_free_cast` skips the mana check. Wand decrements charges on success and self-destructs at zero. Usage gates: mage class OR Magical Secrets (thief/ninja class skill) at mastery >= the wand's spell tier. Wand state (`spell_key`, `charges_remaining`) dual-persisted via `WandNFTItem.persist_wand_state()` to `NFTGameState.metadata` for cross-lifecycle safety. The "Enchanted Wand" NFTItemType is registered without a tracking_token (not AMM-tradeable). 31 tests across `test_recipe_book.TestWandRecipeGeneration` (11), `test_cmd_craft.TestCmdCraftWandEnchant` (6), `test_cmd_zap` (14).
- Item-target spells: spells can target items in addition to actors via three new `target_type` values: `"inventory_item"` (NFT in caster's contents), `"world_item"` (object or exit in caster's room, visibility-filtered through `HiddenObjectMixin`/`InvisibleObjectMixin`), and `"any_item"` (room first, then inventory). Resolution lives in `world/spells/spell_utils.resolve_item_target(caster, target_str, target_type)` and is called from both `cmd_cast.py` and `cmd_zap.py` (so wands of item-targeting spells Just Work). The world_item path delegates to the existing `utils/find_exit_target.py` for visibility-aware lookup, including directional qualifier handling for exits ("door south"). Spells receive the resolved item as `target` in `_execute()` and dispatch via duck-typing rather than isinstance. **Knock** (`world/spells/conjuration/knock.py`) is the worked example: SKILLED conjuration, `target_type="world_item"`, deterministic tier-gated lock_dc ceiling (SKILLED≤15, EXPERT≤20, MASTER≤25, GM=any), unlocks AND opens the target, cancels any pending relock script, works on any LockableMixin subclass via duck-typing (ExitDoor, WorldChest, TrapChest, future). 29 tests across `test_spell_target_resolution` (13) and `test_knock_spell` (16).
...
- NPC hierarchy (post-shopkeeper-refactor): **ShopkeeperNPC** (`typeclasses/actors/npcs/shopkeeper.py`) — abstract base owning `shop_name`, `inventory`, and the abstract pricing interface (`get_buy_price`, `get_sell_price`, `execute_buy`, `execute_sell`, `list_inventory`, `quote_hint`). Concrete siblings: **ResourceShopkeeperNPC** (fungible resources via `AMMService`) and **NFTShopkeeperNPC** (unique items via `NFTAMMService` with proxy PToken/PGold pools). LLM variants **LLMResourceShopkeeperNPC** and **LLMNFTShopkeeperNPC** mix `LLMRoleplayNPC` on top and own the `{shop_commands}` injection. **LLMTrainerNPC** (`llm_trainer.py`) is the LLM-powered trainer base, owning `{train_commands}` injection. **LLMQuestContextMixin** (`typeclasses/mixins/llm_quest_context.py`) is a tiny mixin that injects `{quest_context}` — composed into any LLM NPC that's also quest-aware. **QuestGiverMixin** (`typeclasses/mixins/quest_giver.py`) provides the `quest` command. Concrete multi-role NPCs use explicit composition: BakerNPC = `(LLMQuestContextMixin, QuestGiverMixin, LLMResourceShopkeeperNPC)`; MaraNPC = `(LLMQuestContextMixin, QuestGiverMixin, LLMResourceShopkeeperNPC, TrainerNPC)` — four-way composition (LLM + quest + shop + trainer); Oakwright/Torben/Hendricks/Elena = `(LLMQuestContextMixin, QuestGiverMixin, LLMTrainerNPC)`; LLMGuildmasterNPC = `(LLMQuestContextMixin, LLMMixin, GuildmasterNPC)`. Also: GuildmasterNPC (QuestGiverMixin + BaseNPC, quest system + level caps), BartenderNPC (LLMQuestContextMixin + LLMRoleplayNPC, quest-aware with 5 player states), CombatMob (AI-driven mobs with combat + respawn). Two-tier CmdSet visibility (`call:true()` + `_EmptyNPCCmdSet`). Service NPCs inject role commands. BakerNPC — Bron at Goldencrust Bakery, trades flour/bread, 4 quest states (pitch/active/done/generic) + level gate. Millholm NPC spawner (`world/game_world/zones/millholm/npcs.py`): Rowan (bartender), Bron (baker), Master Oakwright (woodshop trainer+quest), Sergeant Grimjaw (warrior guildmaster), Corporal Hask (warrior trainer), Shadow Mistress Vex (thief guildmaster), Whisper (thief trainer), Archmage Tindel (mage guildmaster), Apprentice Selene (mage trainer), Brother Aldric (cleric guildmaster), Sister Maeve (cleric trainer), Old Silas (beggar LLM NPC in Beggar's Alley), Gemma (jeweller LLM trainer, BASIC), Merchant Harlow (LLM shopkeeper, general store, flour/bread/timber/leather/cloth, `always` speech mode), Farmer Bramble (wheat farmer shopkeeper at Goldwheat Farm), Goodwife Tilly (cotton farmer shopkeeper at Brightwater farmhouse), Ratwick (fence LLM NPC in The Broken Crown, regular memory, NFT shop placeholder), Big Bjorn (lumberjack LLM shopkeeper at Millholm Sawmill, sings Lumberjack Song on arrival, wheat/flour placeholder for wood/timber AMM), Old Buckshaw (trapper LLM shopkeeper at Trapper's Hut in southern woods, coureur des bois personality, wheat placeholder for hide AMM), Grim Thackery (smelter LLM shopkeeper at Millholm Smelter, Four Yorkshiremen personality, trades copper ore/tin ore).
...
- Quest system: FCMQuest base class with step-based progression, FCMQuestHandler (lazy_property on character), quest registry with `@register_quest` decorator, QuestTagMixin on rooms, QuestGiverMixin on NPCs (shared quest command). Templates: CollectQuest, VisitQuest, MultiStepQuest. Guild quests: Warrior Initiation (rat cellar check — instant if already done, else send to clear rats), Thief Initiation (The Shadow Trial — navigate Thieves' Gauntlet, retrieve guild token from Gareth), Mage Initiation (deliver 1 Ruby), Cleric Initiation (feed bread to beggar in Beggar's Alley — QuestTagMixin room trigger). CmdQuests character command for quest log. 87 quest command tests + 21 mixin tests.
...
- World interaction commands:
  ...
  - `sail`: SEAMANSHIP general skill (all classes). Sea travel via dock gateway rooms. See **design/INTERZONE_TRAVEL.md** for the full sail / ship tier / seamanship risk design. Test world docks: Town Dock (off dt6) ↔ Beach Dock (off beach room), `boat_level: 1, food_cost: 1`. 23 tests.
...
- DurabilityDecayService (`typeclasses/scripts/durability_decay_service.py`): global persistent script that applies daily passive decay to every equipped item on puppeted characters. See **design/INVENTORY_EQUIPMENT.md § Durability** for the authoritative reference on wear sources, material tier lifetimes, and the decay service's design. 11 tests.
...
- **NPC system** — BaseNPC + TrainerNPC + GuildmasterNPC complete. QuestGiverMixin extracts quest accept/abandon/view/turn-in into a shared mixin used by GuildmasterNPC, BartenderNPC, and quest-giving shopkeepers. LLMQuestContextMixin injects `{quest_context}` into LLM prompts for any quest-aware LLM NPC. LLM-powered NPCs: BartenderNPC (Rowan, quest-aware with per-player prompt states), BakerNPC (Bron, explicit composition of `LLMQuestContextMixin + QuestGiverMixin + LLMResourceShopkeeperNPC`, AMM shop + quest context). Shopkeeper hierarchy (see above) uses abstract ShopkeeperNPC with ResourceShopkeeperNPC / NFTShopkeeperNPC concrete siblings and LLM variants. CombatMob with AI state machine, combat handler integration, corpse-on-death, and respawn implemented (Rabbit, Wolf, DireWolf). Mobs use same command interface as players. Item/resource drops handled by separate spawn scripts (not traditional loot tables). Future: XP rewards, LLM AI (zero refactoring needed — just swap decision-maker).
- **NPC market maker AMM** — resource trading via XRPL AMM liquidity pools (`ResourceShopkeeperNPC` / `LLMResourceShopkeeperNPC`) and NFT trading via proxy-token pools (`NFTShopkeeperNPC` / `LLMNFTShopkeeperNPC`). Quote→accept flow, instant buy/sell, "sell all" (resources only; NFT shops forbid quantities). Unified pending-quote payload; `CmdShopList`/`CmdShopAccept` are shared across both shop types and polymorphic over the abstract interface on `ShopkeeperNPC`. OfferCreate swap execution with 6-operation accounting. Superuser tools: `amm_check`, `reconcile`, `test_amm_trades`. See **design/ECONOMY.md** for economic design (pricing model, tracker tokens, market tiers, spawn algorithms).
