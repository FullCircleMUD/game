...
- **Combat expansion** — multi-attack (attacks_per_round > 1), combat skill commands (bash, assassinate, etc.), spell casting in combat, combat prompt/status display, LLM-driven mob AI. **Height combat (Phases 1-3 done):** melee blocked across height differentials, missile weapons work cross-height, -4 to-hit for ranged at same height (melee range penalty). `HeightAwareMixin` (`typeclasses/mixins/height_aware_mixin.py`) provides `room_vertical_position` to all objects in vertical space (BaseActor, Corpse, BaseNFTItem, WorldFixture, WorldItem). Death-height behavior: flying actor's corpse falls to ground (height 0) with message, underwater actor's corpse stays at depth. Mid-combat height changes: players get "can't reach" + skip tick, mobs try `_try_match_height()` first (flying mobs chase upward, swimming mobs chase underwater), then retarget to reachable enemy via `_find_reachable_target()`, then flee if none reachable. `_broadcast_height_change()` helper sends room messages when mobs change height mid-combat. Non-repeating action retarget also prefers height-reachable targets. Mob aggro is height-aware: flying mobs (`Condition.FLY`) adjust `room_vertical_position` to match targets before aggro, swimming mobs use `Condition.WATER_BREATHING` for underwater targets (see `AggressiveMixin._try_match_height()`). Flee auto-succeeds when no enemy can melee the fleeing character (all enemies at different height or ranged-only). See `combat/height_utils.py`. 5 retarget tests + 9 height-aware mixin tests. Future phases: mob ranged weapons, spell height interactions, height-aware visibility, get/drop/loot height gating.
