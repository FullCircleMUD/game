"""
ZoneSpawnScript — SUSPECTED DEAD, pending confirmation before deletion.

Retired in favour of the evennia-mob-spawner library
(typeclasses/scripts/ms_load workflow). The original ~390-line
class body is preserved below as a comment block so it can be
restored from this file alone if a regression surfaces.

Once live testing confirms nothing in the running game still
imports `ZoneSpawnScript`, this whole file can be deleted.
"""
